$cities = Get-Content -Path "cities_data.json" -Raw | ConvertFrom-Json
...
foreach ($city in $cities) {
    if ($city.name -eq "Киото") { continue } # Skip Kyoto as it might have specific manual edits
    ...
    # Ensure UTF-8 with BOM for consistent behavior on Windows if needed, but UTF8 is standard for web
